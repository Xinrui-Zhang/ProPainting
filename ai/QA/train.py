...
flags.DEFINE_string(
    "bert_config_file", 'pre_model\\bert_config.json',
    "The config json file corresponding to the pre-trained BERT model. "
    "This specifies the model architecture.")

flags.DEFINE_string("task_name", 'law', "The name of the task to train.")

flags.DEFINE_string("vocab_file", 'pre_model\\vocab.txt',
...
flags.DEFINE_string(
    "init_checkpoint", 'pre_model\\bert_model.ckpt',
...
class LawProcessor(DataProcessor):
    """Processor for the Law data set, train similarity model"""
    ...
    def get_test_examples(self, data_dir):
        """Gets a collection of `InputExample`s for prediction."""
        return self._create_examples(self._read_csv(data_dir, 'test.csv'))

    def get_dev_examples(self, data_dir):
        """Gets a collection of `InputExample`s for the dev set."""
        return self._create_examples(self._read_csv(data_dir, 'dev.csv'))
    ...
    def _create_examples(self, lines):
        ...
        for index, line in enumerate(lines):
            ...
            if len(split_line) == 3 and (split_line[0] == '1' or split_line[0] == '0'):
    ...
    def _read_csv(self, data_dir, file='train.csv'):
        """Read data csv file"""
...
def main(_):
  ...
  if FLAGS.do_predict:
    ...
    with tf.gfile.GFile(output_predict_file, "w") as writer:
      ...
      true_labels = [int(example.label) for example in predict_examples]
